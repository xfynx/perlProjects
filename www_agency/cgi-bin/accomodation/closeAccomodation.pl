...
if($isNotAdminOrUser){
  ...
  $db->do("update accomodation set status='inactive' where id=$id;");
  $db->disconnect();

  print '<html><META http-equiv="refresh" content="0; http://localhost/cgi-bin/accomodation/showAccomodation.pl?id='.$id.'"></html>';
  }
else{
