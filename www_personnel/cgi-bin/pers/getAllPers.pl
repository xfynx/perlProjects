...
if($isLoggedIn){
  my $tmpl = HTML::Template->new(filename => '/var/www/tmpl/pers.tmpl', option => 'value'); #коннектим шаблон
#  my $tmpl = HTML::Template->new(filename => 'D:/xampp/htdocs/perlProject/tmpl/pers.tmpl', option => 'value'); #коннектим шаблон
...
#  $db = DBI->connect("dbi:SQLite:dbname=D:/xampp/htdocs/perlProject/db/personnel_department.sqlite","","") || die $DBI::errstr;
  ...
  my $rows=$db->selectall_arrayref('select 
									...
									where unit.id like personal_data.id_unit order by unit.name, personal_data.last_name;', {Columns=>{}});
									
  my $unitNames=$db->selectall_arrayref("select name from unit;", {Columns=>{}});									
									
  $tmpl->param(ROWS => $rows || [], UNITNAMES=>$unitNames); #привязка
  $db->disconnect();
