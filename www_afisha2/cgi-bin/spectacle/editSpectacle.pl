...
if($isAdmin){
  ...
  my $data=$db->do("update spectacle set description='$description', name='$name', begin_at='$begin_at', duration='$duration', ticket_count=$ticket_count, id_theatre=(select id from theatre where name like '$theatre') where id=$id;");
  $db->disconnect();
