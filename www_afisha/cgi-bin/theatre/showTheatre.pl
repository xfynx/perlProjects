...
if($isNotAdminOrUser){
  ...
  $tmpl = HTML::Template->new(filename => '/var/www/tmpl/showTheatre.tmpl', option => 'value');

  $db = DBI->connect('DBI:mysql:afisha;host=localhost:3306', 'root', '123', { RaiseError => 1 });
  my $data=$db->selectall_arrayref("select id, name, address, place_count from theatre where id = $ID;", {Columns=>{}});
  $db->disconnect();									
